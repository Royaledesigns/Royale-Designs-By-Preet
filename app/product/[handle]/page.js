...
export default async function ProductPage({ params }) {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-12">
      <nav className="text-xs text-forest/50 mb-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        ...
        <div>
          <p className="uppercase text-xs tracking-widest text-gold mb-2">{category?.label}</p>
          ...
          <p className="mt-6 text-forest/70 leading-relaxed">{product.description}</p>

          <div className="flex flex-wrap gap-2 mt-4">
            {product.tags.map((tag) => (
              <span key={tag} className="text-[11px] uppercase tracking-wide border border-forest/20 rounded-full px-3 py-1 text-forest/60">
                {tag}
              </span>
            ))}
          </div>
          ...
          <div className="mt-8 border-t border-forest/10 pt-6 text-sm text-forest/60 space-y-1">
